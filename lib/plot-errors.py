...
def load_data(dataset):
  dataset_safe = dataset.replace(":", "")
  ref = [set() for i in range(xmax)]
  for seq in SeqIO.parse("data/{}.ref.fa".format(dataset_safe), "fasta"):
    for i in range(xmax):
  ...
  for method in methods:
    codons = pd.read_csv("results/{}.{}.codons.csv".format(dataset_safe, method),
                         index_col="AA_Index",
                         nrows=xmax)
    errors = []
